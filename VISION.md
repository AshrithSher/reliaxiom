...
Ring 0 exists today. Ring 1 is **partly** built — ticketing, notifications, and post-mortems
are already interface-backed with stub + real implementations (D-006, D-024); the pollers
already inject their IO (D-015). The remaining Ring-1 seams (telemetry, actions, topology,
change-feed) and the Ring-2 MCP layer are the work in [ROADMAP.md](ROADMAP.md) Part II.
